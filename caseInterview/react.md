...
+ 语法和结构
  + 函数组件：函数式组件，没有this，没有state，没有生命周期，没有this.props和this.state，只能通过props参数来获取数据
  + 类组件：类组件，有this，有state，有生命周期，有this.props和this.state，通过this.props和this.state来获取数据

+ 状态管理
  + 函数组件：在 React Hooks API（如 `useState`、`useReducer` 等）出现之前，函数组件本身没有内部状态。
  + 类组件：通过 `state` 属性和 `setState` 方法来管理内部状态

+ 生命周期钩子
  + 函数组件：可以使用 Hook 来实现类组件中常见的生命周期功能，例如 `useState` 的初始化类似于 `componentDidMount` 和 `componentDidUpdate`，`useEffect` 可以替代多个生命周期方法（包括 `componentDidMount`, `componentDidUpdate`, `componentWillUnmount` 等）。
  + 类组件：具有完整的生命周期钩子方法，如 `constructor`, `componentDidMount`, `componentDidUpdate`, `componentWillUnmount` 等，这些方法在组件的不同阶段被调用，允许开发者在特定时刻执行逻辑。

+ 代码组织与复杂性
  + 函数组件：通常更简洁，尤其在处理简单的 UI 渲染时，由于没有类相关的语法，更容易理解和维护。随着 Hooks 的引入，函数组件也可以承载复杂的业务逻辑。
  + 类组件：对于大型应用中的复杂组件，类组件的结构使得状态和生命周期逻辑能够更好地分离和组织，但在某些情况下可能会增加代码的复杂性和认知负担。
...
## 6、react 的渲染过程中，兄弟节点之间是怎么处理的，也就是key值不一样的时候
...
高阶函数本质上是一个函数，接受一个组件作为参数，并返回一个新的组件。

+ 代码重用：高阶函数可以将通用的逻辑封装在一个函数中，并在多个组件中共享。这样可以避免重复编写相似的代码，提高代码的重用性
+ 逻辑抽象： 通过高阶函数，可以将与组件无关的逻辑从组件中分离出来，使组件更加专注于展示数据和用户交互。这有助于提高组件的可读性和可维护性
+ 组件组合：高阶函数可以将多个功能组合在一起，创建一个新的功能强大的组件。这种组合性能够更灵活地构建复杂的组件，而无需修改现有的组件代码
+ 跨切面关注点（Cross-Cutting Concerns）： 高阶函数可以用来处理跨越多个组件的关注点，如数据获取、权限控制、日志记录等。这使得这些关注点的管理变得更加集中和方便
+ 与 React 生态系统兼容性： 高阶函数是 React 生态系统中的一种常见模式，许多第三方库和框架都提供了基于高阶函数的 API。通过使用高阶函数，你可以更轻松地集成这些库和框架，以实现特定的功能需求
